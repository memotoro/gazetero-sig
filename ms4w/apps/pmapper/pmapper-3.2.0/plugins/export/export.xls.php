...
class ExportXLS extends ExportQuery
{
    ...
    function __construct($json)
    {
        ...
        $this->tempFilePath = $_SESSION['web_imagepath'] . session_id(). ".xls";
        $this->tempFileLocation = $_SESSION['web_imageurl'] . session_id(). ".xls";
        ...
        foreach ($groups as $grp) {
            $worksheet->write($r, 0, utf8_decode($grp->description), $format_title);
            $r++;
            ...
            for ($hi=0; $hi < $hL; $hi++) {
                ...
                if ($headline == "@") {
                    ...
                    $worksheet->write($r, $col, utf8_decode($headline), $format_bold);
                    $col++;
                }
            }
            $r++;
            ...
            foreach ($values as $vList) {
                ...
                foreach ($vList as $v) {
                    ...
                    if (is_object($v)) {
                        ...
                        if ($hyperlink) {
                            $worksheet->write($r, $vcol, utf8_decode($hyperlink[2]));           
                            $vcol++;
                        }
                    } else {
