...
if (($_REQUEST["type"] == "Theme") || ($_REQUEST["type"] == "View")) {
	// If or theme or a view has been passed to be applied :
	if ($_REQUEST["selected"]) {
		$tavLayers = returnListLayers($_REQUEST["selected"], ($_REQUEST["type"] == "Theme"));

		if (($_REQUEST["type"] == "View")) {
			$extentTmp = returnExtent($_REQUEST["selected"]);
			if ($extentTmp) {
	...
	else if ($_REQUEST["groupsandopacities"]) {
